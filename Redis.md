...
```sh
EVAL "
local docinfoList = redis.call('HVALS','GOODOCS:DOCINFO')
local result = {}
for i = 1, #docinfoList do
    local ok, sheet = pcall(cjson.decode, docinfoList[i])
...
" 0
```
* `Redis Insight`는 한글이 깨지므로 터미널에서 `--raw` 옵션을 넣어서 실행 해야 한다.
* `EVAL문`을 `Redis Insight`에서 실행 시키면 한줄로 바꾸어준다.
